...
### ハマりどころ
初回実行時は `Dependency review is not supported on this repository. Please ensure that Dependency graph is enabled` で fail。新規 public リポジトリでも Dependency graph / Dependabot alerts は自動有効化されないケースがあり、**Settings → Code security → Dependabot alerts** を有効化する必要があった。本番導入手順に含めるべき前提条件。
...
1. **Dependency graph / Dependabot alerts の事前有効化**が必要（新規リポジトリで自動有効化されない場合あり）
