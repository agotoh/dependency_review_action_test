...
## Phase 6: 比較・レポート作成
- [x] `docs/report.md` 作成
- [x] 比較表（導入難易度 / 検知力 / PR UX / ノイズ / 費用）を記載
- [x] **各検証 PR の URL をレポートに記載**（スクショは取らず、PR リンクで参照）
  - 正常系 PR: https://github.com/agotoh/dependency_review_action_test/pull/1
  - 異常系 PR: https://github.com/agotoh/dependency_review_action_test/pull/2
...
- [x] Dependency Review Action を本番導入するかの所感・推奨を記載
- [x] main にマージしチーム共有（main 直 push）

## Phase 7: 振り返り

### 想定と実際の差分
- **想定どおりだった点**
  - 2 日以内で Phase 1〜6 を完走
  - Dependency Review Action は異常系 PR を正しくブロック
  - 代替ツール 3 種（bundler-audit / Trivy / Dependabot alerts）も 1 人日以内の軽検証で収まった
- **想定外だった点**
  - 新規 public リポジトリでも Dependency graph / Dependabot alerts が自動有効化されず、初回実行が失敗 → Settings で手動有効化が必要
  - `nokogiri 1.13.9` は Rails 7.2 の依存要件と衝突するため、検証用脆弱 gem には使えず `rubyzip 1.2.2` に変更
  - `aquasecurity/trivy-action@0.28.0` タグは存在せず `@master` に変更
  - `gh run view --log` の出力は Job Summary の内容をすべては含まないため、ログだけで検知結果を判断すると誤読する（実際に 1 度誤報告した）
  - Trivy のカバレッジは Dependency Review Action よりわずかに広い（lodash +1 件）

### `reference-app` 本番導入時の注意点
1. **Dependency graph / Dependabot alerts を事前有効化**する（Settings → Code security）
2. **`fail-on-severity` は moderate を推奨**。low は過剰、high は moderate 脆弱性を見逃す
3. **`comment-summary-in-pr: always`** でレビュー時の視認性を確保
4. Trivy を使う場合は **`aquasecurity/trivy-action` を SHA でピン**する（タグは破綻する可能性あり）
5. **Dependabot alerts は PR ゲートではなく継続監視用**。通知先（Slack 等）を事前に設定
6. **private repo での Dependency Review Action は GitHub Code Security 課金対象**。案 B（Trivy + bundler-audit + Dependabot alerts）なら無料で同等以上のカバレッジが得られる
7. モノレポの場合、`paths:` フィルタで backend / frontend それぞれの依存変更時のみ対応 workflow を発火させるとコスト・時間削減に有効

### 成果物
- 検証リポジトリ: https://github.com/agotoh/dependency_review_action_test
- 計画書: [plan.md](./plan.md)
- タスクリスト（本ファイル）: [tasks.md](./tasks.md)
- レポート: [report.md](./report.md)
- 検証 PR: [#1 正常系](https://github.com/agotoh/dependency_review_action_test/pull/1) / [#2 異常系](https://github.com/agotoh/dependency_review_action_test/pull/2)

- [x] 想定と実際の差分メモ
- [x] 本番プロジェクト導入時の注意点を列挙
